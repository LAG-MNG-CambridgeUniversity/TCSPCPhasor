function [fig, figname] = lifetimefigs_intenonly(dataMatrix_dd,filenamestring,savedir)
% load('jet_new');
%for k=1:n*datanum 
...
%     
    ...
    fig=repmat(inten_dd(cutEdge+1:256-cutEdge,cutEdge+1:256-cutEdge),[1,1,3]);
    % Bound fraction image of donor (multiplied with the intensity image)
    %subplot(datanum,n,count);
%     fig= fracMol_dIn_inten*repmat(binary_image(cutEdge+1:256-cutEdge,cutEdge+1:256-cutEdge),[1,1,3]);
%     labels1=linspace(CLIM_d(1),CLIM_d(end),5);
%     labels=string(labels1); %["1E-9","2E-9","3E-9","4E-9","5E-9"]; %automate this!!!!
    %colormap(map); % Apply the colormap
%     h=colorbar;
%     yTickNumbers = linspace(0,1,5);
%     set(h,'YTick',yTickNumbers,'YTickLabel',labels);
    figname=sprintf('%s segmentation.tif',filenamestring);
